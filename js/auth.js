...
async function handleGoogleSignIn(btnEl) {
    ...
    try {
        ...
        if (err.code !== 'auth/popup-closed-by-user') showToast('Erro ao entrar com Google.');
        btnEl.disabled = false;
        btnEl.innerHTML = original;
    }
}
...
function renderLoginPage() {
    ...
    <div class="min-h-screen flex items-center justify-center" style="background:${bg};">
      <div class="max-w-sm w-full p-8">
        ...
        <button id="loginBtn" class="w-full h-14 rounded-3xl text-white font-bold text-base pulse-tap mb-5" style="background:${colors.authBtnBg};">Entrar</button>
        <div class="flex items-center mb-5">
          <hr class="flex-1" style="border-color:${colors.divider}" />
          <span class="mx-4 text-sm" style="color:${colors.textSecondary}">ou</span>
          <hr class="flex-1" style="border-color:${colors.divider}" />
        </div>
        <button id="googleLoginBtn" class="w-full h-14 rounded-3xl flex items-center justify-center gap-3 pulse-tap" style="background:${colors.authInputFill};border:1px solid ${colors.divider};">
    ...
    document.getElementById('googleLoginBtn').onclick = function () { handleGoogleSignIn(this); };
    document.getElementById('loginBtn').onclick = async () => {
        ...
        if (!email || !pass) { errEl.classList.remove('hidden'); errEl.textContent = 'Preenche todos os campos.'; return; }
        btn.disabled = true; btn.textContent = '…';
        const user = await AuthApiService.login(email, pass);
        if (user && user.token) { authState.setUser(user); window.currentPage = 'chat'; renderChatPage(); }
        else { errEl.classList.remove('hidden'); errEl.textContent = 'Email ou password incorretos.'; btn.disabled = false; btn.textContent = 'Entrar'; }
    };
}
...
function renderRegisterPage() {
    ...
    <div class="min-h-screen flex flex-col" style="background:${bg};">
      ...
      <div class="flex-1 px-8 pt-4 max-w-sm mx-auto w-full">
        ...
        <input id="regName" placeholder="Nome completo" class="w-full h-14 rounded-3xl px-5 text-sm mb-4 border-none outline-none" style="background:${colors.authInputFill};color:${colors.textPrimary};" />
        <input id="regEmail" type="email" placeholder="Email" class="w-full h-14 rounded-3xl px-5 text-sm mb-4 border-none outline-none" style="background:${colors.authInputFill};color:${colors.textPrimary};" />
        <div class="relative mb-4">
          <input id="regPass" type="password" placeholder="Password" class="w-full h-14 rounded-3xl px-5 text-sm border-none outline-none" style="background:${colors.authInputFill};color:${colors.textPrimary};" />
          <button id="toggleRegPass" class="absolute right-4 top-4 w-5 h-5 pulse-tap" style="color:${colors.iconTintSecondary}">
            <span class="icon-mask" style="mask-image:url('assets/icons/svg/eye_closed.svg');-webkit-mask-image:url('assets/icons/svg/eye_closed.svg');background:currentColor;width:20px;height:20px;display:block;"></span>
          </button>
        </div>
        <div class="relative mb-5">
          <input id="regPassConf" type="password" placeholder="Confirmar password" class="w-full h-14 rounded-3xl px-5 text-sm border-none outline-none" style="background:${colors.authInputFill};color:${colors.textPrimary};" />
        ...
        <button id="regBtn" class="w-full h-14 rounded-3xl text-white font-bold text-base pulse-tap mb-5" style="background:${colors.authBtnBg};">Criar conta</button>
        <div class="flex items-center mb-5">
          <hr class="flex-1" style="border-color:${colors.divider}" />
          <span class="mx-4 text-sm" style="color:${colors.textSecondary}">ou</span>
          <hr class="flex-1" style="border-color:${colors.divider}" />
        </div>
        <button id="googleRegBtn" class="w-full h-14 rounded-3xl flex items-center justify-center gap-3 pulse-tap" style="background:${colors.authInputFill};border:1px solid ${colors.divider};">
          <img src="assets/icons/png/google.png" class="w-5 h-5" alt="Google" />
          <span class="font-bold text-sm" style="color:${colors.textPrimary}">Continuar com Google</span>
        </button>
    ...
    document.getElementById('googleRegBtn').onclick = function () { handleGoogleSignIn(this); };
    document.getElementById('regBtn').onclick = async () => {
        ...
        errEl.classList.add('hidden');
        if (!name || !email || !pass || !passConf) { errEl.classList.remove('hidden'); errEl.textContent = 'Preenche todos os campos.'; return; }
        if (pass !== passConf) { errEl.classList.remove('hidden'); errEl.textContent = 'As passwords não coincidem.'; return; }
        btn.disabled = true; btn.textContent = '…';
        const user = await AuthApiService.register(name, email, pass);
        if (user && user.token) { authState.setUser(user); window.currentPage = 'chat'; renderChatPage(); }
        else { errEl.classList.remove('hidden'); errEl.textContent = 'Não foi possível criar a conta.'; btn.disabled = false; btn.textContent = 'Criar conta'; }
    };
}