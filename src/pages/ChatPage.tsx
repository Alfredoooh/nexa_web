...
function subDigit(d: string): string {
  return toSubscript(d);
}
...
body{background:${cls==='dark'?'#121212':'#f4f4f4'};font-family:system-ui,sans-serif;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:14px;padding:20px;min-height:100vh;}
.title{font-size:14px;font-weight:700;color:${cls==='dark'?'#eee':'#222'};margin-bottom:4px;}
.chart{width:100%;max-width:480px;display:flex;align-items:flex-end;gap:10px;height:220px;padding:20px 14px;}
...
.bar{width:100%;max-width:48px;height:var(--h);border-radius:8px 8px 0 0;transform-origin:bottom;animation:grow 700ms ease-out forwards;transform:scaleY(0);}
.label{margin-top:10px;font-size:11px;font-weight:500;color:${cls==='dark'?'#aaa':'#666'};text-align:center;}
@keyframes grow{to{transform:scaleY(1);}}
.legend{display:flex;flex-wrap:wrap;gap:12px;justify-content:center;font-size:13px;color:${cls==='dark'?'#ccc':'#444'};}
...
function pieChartHtml(jsonStr: string, cls: string): string {
  ...
  const paths = slices.map((s, i) => {
    ...
    const x1 = 200 + 140 * Math.cos(startAngle); const y1 = 200 + 140 * Math.sin(startAngle);
    const x2 = 200 + 140 * Math.cos(end); const y2 = 200 + 140 * Math.sin(end);
    const large = sweep > Math.PI ? 1 : 0;
    const mid = startAngle + sweep / 2;
    const tx = 200 + 90 * Math.cos(mid); const ty = 200 + 90 * Math.sin(mid);
    const pct = Math.round(s.value / total * 100);
    const path = `<path d="M200,200 L${x1},${y1} A140,140 0 ${large},1 ${x2},${y2} Z" fill="${color}" stroke="transparent" class="slice"/>
    ${pct >= 5 ? `<text x="${tx}" y="${ty}" text-anchor="middle" dominant-baseline="middle" fill="#fff" font-size="12" font-weight="700">${pct}%</text>` : ''}`;
    startAngle = end;
    return path;
  }).join('');
  ...
  const textColor = cls === 'dark' ? '#eee' : '#333';
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{background:${bg};font-family:system-ui,sans-serif;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:12px;padding:20px;min-height:100vh;}
.title{font-size:14px;font-weight:700;color:${textColor};}
svg{width:min(90vw,300px);height:auto;}
.slice{transition:opacity .2s;cursor:pointer;}.slice:hover{opacity:.85;}
.legend{display:flex;flex-wrap:wrap;gap:10px;justify-content:center;font-size:13px;color:${textColor};}
.legend-item{display:flex;align-items:center;gap:5px;}
.dot{width:10px;height:10px;border-radius:50%;display:inline-block;}
</style></head><body>
${data.title ? `<div class="title">${data.title}</div>` : ''}
<svg viewBox="0 0 400 400">${paths}</svg>
<div class="legend">${legendHtml}</div>
</body></html>`;
}
...
<style>
*{box-sizing:border-box;}
body{margin:0;background:${isDark?'#121212':'#f3f3f3'};display:flex;align-items:center;justify-content:center;min-height:100vh;padding:16px;font-family:Arial,sans-serif;}
.widget{width:min(100%,760px);background:${bg};border:1.5px solid ${border};border-radius:16px;overflow:hidden;}
.hdr{height:42px;display:flex;align-items:center;justify-content:space-between;padding:0 12px 0 14px;background:${headerBg};border-bottom:1px solid ${border};}
.lang{font-size:13px;font-weight:700;color:${titleColor};letter-spacing:.2px;text-transform:uppercase;}
.copy{width:30px;height:30px;border:1px solid ${isDark?'#4a4a4a':'#cfcfcf'};background:${isDark?'#353535':'#fff'};border-radius:10px;cursor:pointer;display:grid;place-items:center;color:${titleColor};}
.body{background:${bg};overflow-x:auto;}
pre{margin:0;padding:16px 16px 16px 52px;color:${text};font-size:14px;line-height:1.7;white-space:pre;font-family:Consolas,Monaco,monospace;counter-reset:line;}
...
<div class="widget">
  <div class="hdr"><div class="lang">${lang}</div><button class="copy" onclick="navigator.clipboard.writeText(document.getElementById('code').innerText)">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
  </button></div>
  <div class="body"><pre><code id="code">${linesHtml}</code></pre></div>
</div>
</body></html>`;
}
...
<script>
let startMs=${countdown && ms > 0 ? ms : 0};
let elapsed=${countdown && ms > 0 ? ms : 0};
...
function fmt(ms){
  ...
  return [h,m,s,cs].map(x=>String(x).padStart(2,'0')).join(':');
}
function tick(){
  const now=performance.now();
  if(isCountdown){elapsed=Math.max(0,startMs-(now-startTime));if(elapsed===0){running=false;}}
  else{elapsed=startMs+(now-startTime);}
  document.getElementById('d').textContent=fmt(elapsed);
  if(running)raf=requestAnimationFrame(tick);
}
function toggle(){
  if(running){running=false;cancelAnimationFrame(raf);startMs=elapsed;}
  else{running=true;startTime=performance.now();tick();}
}
...
function mathGraphHtml(jsonStr: string, cls: string): string {
  ...
  const curveColor = '#6cb6ff';
  return `<!DOCTYPE html><html><head><meta charset="UTF-8">
<script src="https://cdnjs.cloudflare.com/ajax/libs/mathjs/11.8.0/math.min.js"><\/script>
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{background:${bg};display:flex;align-items:center;justify-content:center;min-height:100vh;padding:16px;flex-direction:column;gap:8px;font-family:Arial,sans-serif;}
...
const f=math.compile(${JSON.stringify(expr)});
const pts=[];
for(let i=0;i<=400;i++){
  const x=xMin+(xMax-xMin)*i/400;
  try{const y=f.evaluate({x});if(isFinite(y))pts.push([x,y]);}catch(e){}
}
if(!pts.length){ctx.fillStyle='#888';ctx.fillText('Sem dados',20,20);}
else{
...
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{background:${cls==='dark'?'#121212':'#f4f4f4'};min-height:100vh;display:grid;place-items:center;font-family:system-ui,sans-serif;padding:16px;}
.card{width:min(90vw,500px);height:380px;background:${bg};border-radius:20px;overflow:hidden;position:relative;}
svg{width:100%;height:100%;}
.node rect{stroke-width:1.8;rx:8;}
.node text{fill:#fff;font-size:12px;font-weight:600;text-anchor:middle;dominant-baseline:central;pointer-events:none;}
.link{stroke:${cls==='dark'?'#666':'#bbb'};stroke-width:1.8;fill:none;stroke-linecap:round;}
</style></head><body>
<div class="card"><svg id="svg"><g id="g"></g></svg></div>
<script>
const data=${jsonStr};
const svg=document.getElementById('svg');const g=document.getElementById('g');
const LW=160,NH=36,NP=24,VS=NH+NP;
function gh(n,col){if(col[n.id]||!n.children||!n.children.length)return VS;return n.children.reduce((s,c)=>s+gh(c,col),0);}
function layout(n,x,y,col){const pos={};const h=gh(n,col);pos[n.id]={x,y:y+h/2};
if(!col[n.id]&&n.children){let cy=y;n.children.forEach(c=>{const ch=gh(c,col);Object.assign(pos,layout(c,x+LW,cy,col));cy+=ch;});}return pos;}
function render(){
  g.innerHTML='';
  ...
  function drawLinks(n){if(!n.children)return;n.children.forEach(c=>{const f=pos[n.id],t=pos[c.id];if(f&&t){
    ...
    p.setAttribute('class','link');g.appendChild(p);}drawLinks(c);});}
  function drawNodes(n){const{x,y}=pos[n.id]||{x:0,y:0};
    const ng=document.createElementNS('http://www.w3.org/2000/svg','g');ng.setAttribute('class','node');
    const tw=Math.max(70,n.label.length*7+24),th=NH;
    const rect=document.createElementNS('http://www.w3.org/2000/svg','rect');
    rect.setAttribute('x',-tw/2);rect.setAttribute('y',-th/2);rect.setAttribute('width',tw);rect.setAttribute('height',th);
    rect.setAttribute('fill',n.color||'#6F5AF6');rect.setAttribute('rx',8);ng.appendChild(rect);
    const txt=document.createElementNS('http://www.w3.org/2000/svg','text');txt.textContent=n.label;ng.appendChild(txt);
...
body{background:#f2f2f2;display:grid;place-items:center;min-height:100vh;padding:16px;font-family:Arial,sans-serif;}
.widget{width:min(92vw,420px);background:#111318;border-radius:24px;overflow:hidden;}
.info{display:flex;align-items:center;justify-content:space-between;padding:20px 16px 8px;}
.left{display:flex;align-items:center;gap:12px;}
.logo{width:44px;height:44px;border-radius:50%;background:#1e2128;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:#fff;}
.name{font-size:15px;font-weight:700;color:#fff;}
.sym{font-size:12px;color:#555;margin-top:2px;}
.price{font-size:22px;font-weight:800;color:#fff;text-align:right;}
.chg{display:inline-block;font-size:12px;font-weight:700;padding:3px 8px;border-radius:6px;margin-top:4px;}
.up{background:#0d2e1a;color:#22c55e;}.down{background:#2e0d0d;color:#ef4444;}
canvas{display:block;width:calc(100% - 20px);margin:0 10px;height:130px;}
.tfs{display:flex;justify-content:center;gap:4px;padding:8px 16px 16px;}
...
.tf.a{background:#1e2128;color:#fff;}
</style></head><body>
<div class="widget">
  <div class="info">
    <div class="left">
      <div class="logo">${(data.symbol||data.name||'?').slice(0,3)}</div>
      <div><div class="name">${data.name||'Ativo'}</div><div class="sym">${data.symbol||''} · ${(data.type||'').toUpperCase()}</div></div>
    </div>
    <div>
      <div class="price">${data.price_str || (data.price ? '$'+Number(data.price).toFixed(2) : '—')}</div>
      ${data.change != null ? `<div class="chg ${data.change>=0?'up':'down'}">${data.change>=0?'▲ +':'▼ '}${Math.abs(data.change).toFixed(2)}%</div>` : ''}
    </div>
  </div>
  <canvas id="c"></canvas>
  <div class="tfs"><button class="tf a">1D</button><button class="tf">1S</button><button class="tf">1M</button><button class="tf">3M</button><button class="tf">1A</button></div>
</div>
<script>
const c=document.getElementById('c');const ctx=c.getContext('2d');
const isUp=${(data.change||0)>=0};
const color=isUp?'#22c55e':'#ef4444';
let prices=${JSON.stringify(data.history||[])};
if(!prices.length){
  const base=${data.price||100};let p=base*0.9;
  for(let i=0;i<42;i++){p+=(Math.random()-(isUp?.46:.54))*p*.02;prices.push(Math.max(p,.01));}
  prices.push(base);
}
function draw(){
  const W=c.offsetWidth||400,H=130;
  c.width=W*devicePixelRatio;c.height=H*devicePixelRatio;c.style.height=H+'px';
  ctx.scale(devicePixelRatio,devicePixelRatio);
  const min=Math.min(...prices),max=Math.max(...prices),range=max-min||1;
  const pad=10;const pw=W-pad*2,ph=H-pad*2;
  const pts=prices.map((v,i)=>({x:pad+i/(prices.length-1)*pw,y:pad+ph-(v-min)/range*ph}));
  const grad=ctx.createLinearGradient(0,0,0,H);
  grad.addColorStop(0,color+'55');grad.addColorStop(1,color+'00');
  ctx.beginPath();ctx.moveTo(pts[0].x,H);pts.forEach(p=>ctx.lineTo(p.x,p.y));ctx.lineTo(pts.at(-1).x,H);ctx.closePath();
...
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{background:#f0f2f5;display:grid;place-items:center;min-height:100vh;font-family:system-ui,sans-serif;padding:16px;}
.card{width:min(90vw,400px);height:340px;background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 8px 24px rgba(0,0,0,.1);}
#map{width:100%;height:280px;}
.footer{padding:10px 14px;display:flex;align-items:center;gap:8px;}
.pin{color:#FF3B30;font-size:18px;}
...
<div class="card">
  <div id="map"></div>
  <div class="footer"><span class="pin">📍</span><span class="name">${loc}</span></div>
</div>
<script src="https://unpkg.com/maplibre-gl@4.3.2/dist/maplibre-gl.js"><\/script>
<script>
const map=new maplibregl.Map({container:'map',style:'https://tiles.openfreemap.org/styles/liberty',center:[${lng},${lat}],zoom:${zoom},attributionControl:false});
...
<style>
*{box-sizing:border-box;margin:0;padding:0;}
:root{--bg:${isDark?'#121212':'#f4f4f4'};--card:${isDark?'#1b1b1b':'#fff'};--border:${isDark?'#333':'#e0e0e0'};--text:${isDark?'#eee':'#222'};--muted:${isDark?'#888':'#999'};--sel:${isDark?'#7c3aed':'#6F5AF6'};--today:${isDark?'#2a2a40':'#ede9ff'};}
body{background:var(--bg);display:grid;place-items:center;min-height:100vh;padding:16px;font-family:system-ui,sans-serif;}
.cal{width:min(92vw,380px);background:var(--card);border:1.5px solid var(--border);border-radius:20px;padding:16px;}
...
.nav{background:${isDark?'#2a2a2a':'#f5f5f5'};border:none;border-radius:10px;width:32px;height:32px;cursor:pointer;color:var(--text);font-size:18px;display:grid;place-items:center;}
.wdays{display:grid;grid-template-columns:repeat(7,1fr);margin-bottom:6px;}
.wdays span{text-align:center;font-size:11px;font-weight:600;color:var(--muted);padding:3px 0;}
.grid{display:grid;grid-template-columns:repeat(7,1fr);gap:3px;}
.day{aspect-ratio:1;display:flex;align-items:center;justify-content:center;border-radius:50%;font-size:13px;color:var(--text);cursor:pointer;position:relative;}
.day.other{opacity:.3;cursor:default;}
.day.today{background:var(--today);color:${isDark?'#a78bfa':'#6F5AF6'};font-weight:700;}
.day.sel{background:var(--sel);color:#fff;font-weight:700;}
.day.has::after{content:'';position:absolute;bottom:2px;width:4px;height:4px;border-radius:50%;background:var(--sel);}
.day.sel.has::after{background:#fff;}
.evts{margin-top:12px;border-top:1px solid var(--border);padding-top:10px;}
.elbl{font-size:11px;font-weight:700;color:var(--muted);margin-bottom:8px;text-transform:uppercase;}
.eitem{display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:10px;background:${isDark?'#252535':'#f7f6ff'};margin-bottom:5px;}
.edot{width:9px;height:9px;border-radius:50%;flex-shrink:0;}
.ename{font-size:13px;font-weight:600;color:var(--text);}
.etime{font-size:11px;color:var(--muted);}
.none{font-size:13px;color:var(--muted);text-align:center;padding:8px;}
</style></head><body>
<div class="cal">
  <div class="hdr"><button class="nav" id="pv">‹</button><div class="title" id="ttl"></div><button class="nav" id="nx">›</button></div>
  <div class="wdays"><span>Dom</span><span>Seg</span><span>Ter</span><span>Qua</span><span>Qui</span><span>Sex</span><span>Sáb</span></div>
  <div class="grid" id="gr"></div>
  <div class="evts"><div class="elbl">Eventos do dia</div><div id="el"></div></div>
</div>
<script>
const ev=${eventsJson};
const months=['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
const td=new Date();td.setHours(0,0,0,0);
let cur=new Date(td.getFullYear(),td.getMonth(),1);
let sel=dk(td.getFullYear(),td.getMonth(),td.getDate());
function dk(y,m,d){return y+'-'+String(m+1).padStart(2,'0')+'-'+String(d).padStart(2,'0');}
function render(){
  const y=cur.getFullYear(),m=cur.getMonth();
  document.getElementById('ttl').textContent=months[m]+' '+y;
  const fd=new Date(y,m,1).getDay(),dim=new Date(y,m+1,0).getDate(),prev=new Date(y,m,0).getDate();
  const gr=document.getElementById('gr');gr.innerHTML='';
  for(let i=fd-1;i>=0;i--){const d=document.createElement('div');d.className='day other';d.textContent=prev-i;gr.appendChild(d);}
  for(let d=1;d<=dim;d++){
    const key=dk(y,m,d),date=new Date(y,m,d);
    const isToday=date.getTime()===td.getTime(),isSel=key===sel,hasEv=!!(ev[key]&&ev[key].length);
    const el=document.createElement('div');
    el.className='day'+(isToday?' today':'')+(isSel?' sel':'')+(hasEv?' has':'');
    el.textContent=d;el.onclick=()=>{sel=key;render();};gr.appendChild(el);
  }
  const used=fd+dim,rem=used%7===0?0:7-used%7;
  for(let d=1;d<=rem;d++){const el=document.createElement('div');el.className='day other';el.textContent=d;gr.appendChild(el);}
  renderEvents();
}
function renderEvents(){
  const el=document.getElementById('el');el.innerHTML='';
  const dayEv=ev[sel];
  if(!dayEv||!dayEv.length){el.innerHTML='<div class="none">Nenhum evento</div>';return;}
  dayEv.forEach(e=>{el.innerHTML+=\`<div class="eitem"><div class="edot" style="background:\${e.color||'#6F5AF6'}"></div><div><div class="ename">\${e.name}</div>\${e.time?'<div class="etime">'+e.time+'</div>':''}</div></div>\`;});
}
document.getElementById('pv').onclick=()=>{cur.setMonth(cur.getMonth()-1);render();};
...
<style>
*{margin:0;padding:0;box-sizing:border-box;}
body{background:${isDark?'#121212':'#f3f3f3'};display:flex;align-items:center;justify-content:center;min-height:100vh;padding:16px;font-family:Arial,sans-serif;}
.paper{width:min(92vw,620px);border:1px solid ${border};background:${bg};overflow:hidden;}
</style></head><body>
<div class="paper">
<svg xmlns="http://www.w3.org/2000/svg" width="100%" height="${totalH}" viewBox="0 0 620 ${totalH}">
  ${rulesHtml}
  <line x1="56" y1="0" x2="56" y2="${totalH}" stroke="${marginColor}" stroke-width="1"/>
  ${linesHtml}
</svg>
</div>
</body></html>`;
        ...
        <div style={{ borderRadius: 14, border: `1.5px solid ${cbBorder}`, overflow: 'hidden', marginTop: 8, marginBottom: 8, background: cbBg }}>
          <div style={{ background: cbHeader, padding: '0 12px', height: 38, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${cbBorder}` }}>
            ...
            <button
              ...
              style={{ background: isDark ? '#353535' : '#fff', border: `1px solid ${isDark ? '#4a4a4a' : '#cfcfcf'}`, borderRadius: 8, cursor: 'pointer', padding: '3px 8px', color: cbTitle, fontSize: 11 }}
            >
              Copiar
            </button>
          </div>
...
// ─── Popup Menu Flutuante ─────────────────────────────────────────────────────
function FloatingPopupMenu({
  open, onClose, isDark, anchorRight,
  items,
}: {
  open: boolean;
  onClose: () => void;
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            style={{
              position: 'fixed',
              ...(anchorRight ? { right: 14 } : { left: 14 }),
              bottom: anchorRight ? undefined : 108,
              top: anchorRight ? 56 : undefined,
              zIndex: 199,
              background: menuBg,
              borderRadius: 14,
              boxShadow: isDark
            ...
            {items.map((opt, i) => (
              <button
                ...
                style={{
                  ...
                  fontSize: 15, color: textClr, textAlign: 'left',
                }}
              >
                <img
                  src={opt.icon}
                  ...
                  height={18}
                  alt=""
                  style={{ filter: isDark ? 'invert(1)' : 'none', flexShrink: 0 }}
                />
...
export default function ChatPage() {
  ...
  const addMenuItems = [
    ...
    { icon: '/assets/icons/svg/magnifying_glass_outline.svg',  label: 'Pesquisar',   action: () => {} },
  ...
  const topMenuItems = [
    ...
    { icon: '/assets/icons/svg/settings.svg',   label: 'Definições',  action: () => {} },
      ...
      <AnimatePresence>
        {optionsConv && (
          <>
            ...
            <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 340, damping: 34 }} style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 201, background: isDark ? '#1C1C1E' : '#fff', borderRadius: '14px 14px 0 0', padding: '8px 0 32px' }}>
